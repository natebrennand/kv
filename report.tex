...
Preliminary results show lower-than-expect performance.
...
As highlighted in Figure 1, unikernels have a greatly reduced overhead when accessing resources when compared to applications running on Linux.
This approach brings some great benefits, such as zero\-copy IO and no scheduler\-induced delay in response.
...
Arrakis in particular benefits from increased support for virtualization in new hardware, which allows it to effectively run all applications in kernel space and achieve seperation using hardware instructions, similar to how MirageOS achieves seperation through Xen.
