...
The Redis benchmark suite was used to examine the throughput of the implemented GET, SET, MSET, and PING commands.
Three aspects of scalability were tested: raw request volume, request parallelization, and request pipelining.
To test scalability with raw volume, a benchmark setup would have either a total of one million requests or five million of each command.
For parallelization, the remote host would set up a group of 10, 100, 500, one thousand clients.
Regarding pipelining, requests with either pipelined with three requests per packet sent, or with no pipelining at all.
...
Neither increasing the number of total requests or enabling pipelining had a significant effect on the relative throughput of Keyvee when compared to Redis, meaning that Keyvee was scalable with large numbers of requests.
However, scaling the number of concurrent clients had a profound effect on throughput.
Across all the implemented commands, Keyvee performed just as well, if not better, than Redis when only ten clients were connecting at once.
Beyond 10 clients, Keyvee's performance was consistently poorer than Redis, degrading logarithmically with the increase in clients and leveling off to roughly half of Redis' performance.
