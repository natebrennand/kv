...
All these three unikernel implementations (HaLVM, LING and Mirage) have in common that you are limited to one functional language exclusively and when compiled the resulting image is built to run on top of the Xen hypervisor.
...
However, it support Linux ABI, and contains necessary functionality to run Java and POSIX applications which allows OSv to run some existing codebases and get some of the performance benefit Mirage enjoys. To get zero-copy I/O specific non-POSIX OSv API's need to be used. 
