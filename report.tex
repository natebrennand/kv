...
Our Xen hypervisor installation was coordinated by a Ubuntu Server 14.04 host that serves as the management domain to manage the other hosts running on the Hypervisor.
The Mirage installation running Keyvee was modified to use Redis's communication protocols so that we could use the same benchmark for all test setups.
...
We used a vanilla Redis 2.8 installation on Xen as the comparison environment to Keyvee.
The test clients were hosted on a Macbook Pro and connected via a network bridge.

Using the benchmark suite, three aspects of scalability were tested: total number of requests, number of clients making making requests in parallel, and pipelining.
Each test used a combination of either one million or five million requests;e ither ten, one hundred, five hundred, or one thousand parallel clients; and a pipeline of either one (effectively no pipelining) or 3 requests, for a total of 16 different tests.
All tests used a key space of 1 key with keys 20 bytes large.

\subsection{Throughput Performance}

Test results showed that for ten parallel clients, Keyvee performed just as well, if not better, than Redis.
Beyond 10 clients, Keyvee's performance was consistently poorer than Redis, with performance degrading logarithmically with the increase in clients, leveling off to roughly half of Redis' performance.
Upon further observation and testing, it was found that the Lightweight thread architecture  used by Mirage to handle simultaneous connections was the cause of the performance loss. We concluded that the LWTs was not a mature technology when compared to Kqueue and Epoll that Redis relies on.
...
While similar, if not superior throughput was achieved in tests having few parallel clients, a scaling issue was found when Keyvee was subjected to larger parallel loads.
