...
$ I am assuming at the moment we are using the current tests on gist$

A total of six benchmark setups were tested. Both environments used a key space of 1 key of 5 bytes. Each setup had 10, 100, or 500 clients communicating with the environment, making a total of 1 million requests for the first two setups and one hundred thousand requests for the latter. Each packet sent from a client would either contain one or three requests.

\subsection{Throughput Performance}

Test results showed that for few (10 or less) parallel clients, Keyvee performed just as well, if not better, than Redis.
