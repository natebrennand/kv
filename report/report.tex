...
This provides a simple consistent platform for developers to write their applications against and running a full stack machine on top of hypervisors is understandable in an historical context but it is not necessarily the most efficient.
One downside for example is extra overhead that we propose can be eliminated.
Key-value datastores are a common component of web infrastructure and have various use cases, the most prominent datastore today being Redis.
...
Applications can be developed and then compiled into unikernels using a toolchain, like Mirage OS for example, and can be deployed directly on top of a Xen hypervisor.
We've designed a Mirage OS\cite{mirage} unikernel implementation of the popular Redis\cite{redis} in-memory key-value store.
We describe the advantages and disadvantages that come with such a system.
A unikernel implementation of Redis has performance and security benefits as well as properties that allow fast creation and deployment and easier manipulation in scaling up and down according to allocated resources.
We highlight the performance characteristics of this new approach and compare to Redis.
Preliminary results showed lower than expected performance.
We expect with more profiling and testing we can identify hotspots in our codebase and garner better performance from the Keyvee implementation.
\end{abstract}
